...
public interface HDCTRepo extends JpaRepository<HoaDonChiTiet, Integer> {
    @Query(value = "Select hdct.id, hd.ma_hoa_don, ctsp.id, ctsp.id_sp, ctsp.id_chat_lieu, ctsp.id_mau_sac, ctsp.id_size, hdct.gia_ban, hdct.so_luong_mua, hdct.tong_tien, hdct.trang_thai\n" +
            "from hdct inner join ctsp on hdct.id_ctsp = ctsp.id\n" +
            "inner join hoa_don hd on hdct.id_hoa_don = hd.id", nativeQuery = true)
    List<HoaDonChiTiet> getHoaDonChiTietInterface();
